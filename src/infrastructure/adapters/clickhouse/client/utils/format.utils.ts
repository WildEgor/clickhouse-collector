...
import { IClickhouseOptimizedJSONInsertFormat, TJSONFormatRow } from '../clickhouse.interfaces';
...
export class ClickhouseFormatUtils {
	public static jsonRowsToInsertFormat(
		...
		const values = rows.map((row) =>
			keys.map((key) => ClickhouseFormatUtils.getSimpleValidatedValue(key, row[key])),
		);
		return {
			keys,
			values,
	...
	public static getSimpleValidatedValue(
		...
		if (Array.isArray(value)) {
			return `[${value.map(ClickhouseFormatUtils.getSimpleValidatedValue).join(',')}]`;
		}
		...
		if (typeof value === 'string' && specialOps.has(value)) {
		...
		// eslint-disable-next-line @typescript-eslint/restrict-template-expressions
		throw new ClickhouseInsertError(`Unknown type of key-value [${key}:${value}]`);
	}
}
